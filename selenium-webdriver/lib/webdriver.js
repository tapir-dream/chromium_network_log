  ...
  executeScript(script, var_args) {
   ...
   return this.schedule(
    ...
    setNetworkConditions (opts) {
      var key = (typeof opts == 'string') ? 'network_name' : 'network_conditions';
      return this.schedule(
        new command.Command(command.Name.SET_NETWORK_CONDITIONS).
            setParameter(key, opts),
        'WebDriver.setNetworkConditions()');
    };

    getNetworkConditions () {
      return this.schedule(
        new command.Command(command.Name.GET_NETWORK_CONDITIONS),
        'WebDriver.getNetworkConditions()');
    };

    deleteNetworkConditions () {
      return this.schedule(
        new command.Command(command.Name._NETWORK_CONDITIONS),
        'WebDriver.deleteNetworkConditions()');
    };
    // ============ Tapir END =============
