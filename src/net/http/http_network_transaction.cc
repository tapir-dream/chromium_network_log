...
// ========== Tapir ADD ==========
#include "net/http/network_log.h"

// ========== Tapir ADD ==========
namespace net {

Network_log* Network_log::m_pInstance = NULL;
Network_log::Network_log() { }
Network_log::~Network_log() { Network_log::m_pInstance = NULL; }
// ========== Tapir END ==========
...
void HttpNetworkTransaction::AddHttpHeadersLog() {
  ...
  base::StringAppendF(&output, "\"method\": \"%s\", ", request_->method.c_str());
  base::StringAppendF(&output, "\"version\": %d.%d, ",
                      response_.headers->GetHttpVersion().major_value(),
                      response_.headers->GetHttpVersion().minor_value());
  base::StringAppendF(&output, "%s", "\"request_headers\": ");
  base::StringAppendF(&output, "%s", request_headers_.HeaderToJSON().c_str());
  base::StringAppendF(&output, "%s", ", \"response_headers\": ");
  base::StringAppendF(&output, "%s", response_.headers->HeaderToJSON().c_str());
  ...
  Network_log* network_log = Network_log::Instance();
  network_log->setValue(output);
}
  
// =========== Tapir ADD ==========
void HttpNetworkTransaction::AddRequestSizeLog() {
  LoadTimingInfo load_timing_info;
  GetLoadTimingInfo(&load_timing_info);
  const base::TimeTicks kNullTicks;
