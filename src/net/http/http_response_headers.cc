...
// =============== Tapir ADD =======================
std::string HttpResponseHeaders::HeaderToJSON() {
  std::string output;
  std::string a("\"");
  std::string b("\\\"");
  ...
  while (EnumerateHeaderLines(&iter, &key, &value)) {
    str_replace(key, a, b);
    str_replace(value, a, b);
    base::StringAppendF(&output, "\"%s\": \"%s\",", key.c_str(), value.c_str());
  }
...
int HttpResponseHeaders::str_replace(std::string &str, const std::string &src, const std::string &dest)
{
  int counter = 0;
